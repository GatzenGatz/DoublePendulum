...
\noindent This note analyzes a double compound pendulum consisting of two rods of equal length and uniform mass distribution. The kinetic and potential energies are given by
\begin{align}
...
\end{align}
Using the Euler-Lagrange equations we could solve for the Equations of Motion in the form of Lagrangian mechanics. By this we would obtain two second-order differential equations. But solving for the Equations of Motion in Hamilton mechanics is better for simulating, because we only get first-order differential equations, which can more be solved more accurately numerically. Therefore we perform a Legendre-Transform on the Lagrangian, which turns it into the Hamiltonian.\\
...
It's trivial to see, that the double pendulum stays bounded on the $\varphi_1$-$\varphi_2$-plane. This can be useful when analysing the phase-space of the system.
\end{document}
